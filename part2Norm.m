function part2Norm(app,D,Rad,compound_theory,Cf,C_v,rootdir,resultpos,sm_val,noise_flag)
    for smooth = 1:length(sm_val)
            sm = sm_val(smooth);
            clc;
            close all;
            clearvars -except RMSVals app sm C_v Cf smooth resultpos sm_val rootdir Rad compound_theory  D noise_flag;
            format shortE;
    ...
    %                                 trap_approx_int_pointwise_at_0,exact_int_pointwise_at_0,fit_Ct,exact_Ct]
                                [trap_approx_int_pointwise, exact_int_pointwise, trap_approx_int, exact_int, tcoords_top, fit_grad_t, exact_grad_t,...
                                 trap_approx_int_pointwise_at_0, exact_int_pointwise_at_0, fit_C, exact_Ct] =top_integration(evap_file1,hRR1,Mu,lc,R,Z,rho_gridfit,fit_gradr,fit_gradz,...
                                                rho,rho_gradr,rho_gradz,radius,fsize,path3,D);
                                ...
                                [trap_approx_int_pointwise_cv,exact_int_pointwise_cv,trap_approx_int_cv(check),exact_int_cv(check),tcoords_side,fit_grad_ts,exact_grad_ts,fit_Cs,exact_Cs] =...
                                    side_integration(evap_file2,hRR1,Mu,lc,R,Z,rho_gridfit,fit_gradr,fit_gradz,...
                                                rho,rho_gradr,rho_gradz,radius,fsize,path3,D);
                        ...
                        app.CVFluxRMSTextArea.Value = [TextAreaText1';averageRMS];
